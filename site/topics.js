...
window.TOPICS = [
...

  {
    ...
    summary:    "Interview-focused ML foundations: bias-variance tradeoff, L1/L2 regularization, precision/recall/ROC-AUC metrics, Random Forests vs XGBoost/LightGBM, SVM kernel trick, logistic regression, K-means, PCA, cross-validation, class imbalance (SMOTE), feature engineering (encoding, scaling, missing values), and hyperparameter tuning (Bayesian optimization).",
    subtopics:  [
      ...
      "Gradient Boosting — XGBoost vs LightGBM, leaf-wise vs level-wise",
      "SVM — Max-margin hyperplane, kernel trick (RBF, Polynomial)",
      ...
      "Cross-Validation — Stratified k-fold, time-series split, leakage",
      "Class Imbalance — class_weight, SMOTE, threshold tuning",
      "Categorical Encoding — One-hot, Target Encoding, Hashing",
      "Feature Scaling — StandardScaler, RobustScaler (outliers), when trees don't need it",
...

  {
    ...
    icon:       "🧠",
    difficulty: "intermediate",
    summary:    "Neural networks from first principles: activation functions (ReLU vs GELU), backpropagation & vanishing gradients, Adam/AdamW optimizers, learning rate scheduling, BatchNorm vs LayerNorm, Dropout, CNN architectures (ResNet skip connections), LSTMs (gates & cell state), attention mechanism (Q·K·V), transfer learning strategies, mixed precision training, and production debugging.",
    subtopics:  [
      ...
      "CNN Architectures — ResNet (skip connections), InceptionNet, EfficientNet (compound scaling)",
      "LSTM Gates — Forget, input, output gates; cell state as gradient highway",
      "GRU — Simplified LSTM; fewer parameters",
      "Attention Mechanism — Q/K/V scaled dot-product, scaling by √dₖ",
      "Multi-Head Attention — Parallel heads for different subspaces",
